...
app.use((req, res, next) => {
  console.log(`Incoming request on path: ${req.path}`);
  next();
});
...
app.delete("/delete/*", async (req, res, next) => {
  try {
    ...
    const deletedFileName = await deleteFile(fileName);
    res.status(200).json({
...
app.use((err, req, res, next) => {
  // log the request path and error message for debugging
  console.error(`Error occurred on path ${req.path}: ${err.message}`);
  res.status(500).json({
    error: err,
